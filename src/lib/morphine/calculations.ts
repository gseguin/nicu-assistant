...
/**
 * Calculate a compounding weaning schedule.
 * Each step reduces by a percentage of the current dose (exponential decay).
 */
export function calculateCompoundingSchedule(
  weightKg: number,
  maxDoseMgKgDose: number,
  decreasePct: number
): WeanStep[] {
  ...
  for (let i = 0; i < config.stepCount; i++) {
    ...
    if (i === 0) {
      ...
      const previousDose = steps[i - 1].doseMg;
      const doseMg = previousDose * (1 - decreasePct);
      const reductionMg = previousDose - doseMg;
      steps.push({
        step: stepNum,
        doseMg,
        doseMgKgDose: doseMg / weightKg,
