# Requirements Archive: v1.15.1 iOS Polish & Drawer Hardening
...
- [x] **DRAWER-05**: `InputDrawer.svelte` `.input-drawer-sheet` exposes `--ivv-bottom` and `--ivv-max-height` CSS custom properties driven by the singleton; no per-calculator prop plumbing across the six call sites
- [x] **DRAWER-06**: Sheet `max-height` becomes `calc(var(--ivv-max-height, 80dvh))` so when the keyboard is up the sheet shrinks to fit the available `visualViewport.height − 16px` (eyebrow + first input remain visible)
- [x] **DRAWER-07**: Sheet `padding-bottom` becomes `max(env(safe-area-inset-bottom, 0px), var(--ivv-bottom, 0px))` so when the keyboard is up the sheet's bottom edge sits flush above the keyboard top with ≥ 8 px clearance; when the keyboard is down the existing safe-area-inset-bottom clearance is preserved
- [x] **DRAWER-08**: `transform`/`max-height` modifications apply ONLY to the inner `.input-drawer-sheet`, never to the outer `<dialog>` element, so top-layer positioning rules and `<dialog>` accessibility semantics are preserved (and the existing SelectPicker dialog inside the drawer is unaffected)
...
- [x] **DRAWER-10**: `prefers-reduced-motion: reduce` honored — when set, sheet sizing/positioning snaps without transition (consistent with v1.6 / Phase 42.1 reduced-motion contract); when unset, transition uses an existing scoped CSS rule (no new global transitions)
- [x] **DRAWER-11**: Existing `md:hidden` rule on the drawer is preserved (drawer never appears on tablet/desktop breakpoints) so iPad split-keyboard cases are out of scope per architectural design
- [x] **DRAWER-12**: Existing `<dialog>` `showModal()` + top-layer + Esc-to-close + focus-trap + focus-restore behaviors are preserved verbatim — no replacement with `position: fixed` or other anti-pattern (reference: PITFALLS.md anti-feature list)
...
- [x] **DRAWER-TEST-02**: Vitest component test on `InputDrawer.svelte` asserts that `style="--ivv-bottom: …px; --ivv-max-height: …px"` is applied to `.input-drawer-sheet` and updates when the mock visualViewport dispatches a resize
...
- [x] **REL-01**: `package.json` version bumped (1.15.0 → 1.15.1 → 1.16.0 → 1.16.1 outside this milestone via quick task 260430-cvl; AboutSheet reflects shipped version via `__APP_VERSION__`). Note: minor bump to 1.16.0 happened before milestone close; v1.15.1 was shipped under v1.16.x.
- [x] **REL-02**: PROJECT.md Validated list updated with v1.15.1 entries at milestone completion; Active section cleared; Last updated footer bumped
- [x] **REL-03**: REQUIREMENTS.md traceability table all v1.15.1 IDs flipped to ✓ Validated (DRAWER stragglers retroactively flipped per 49-02-SUMMARY); ROADMAP.md Progress rows marked Complete; v1.15.1 archived to `.planning/milestones/v1.15.1-{REQUIREMENTS,ROADMAP,phases}/` per existing pattern
- [~] **REL-04**: Clinical gate green on automated tests (svelte-check 0/0, vitest 454/454 at phase-49 close, Playwright chromium + webkit-iphone projects + axe suite green). SMOKE-01..10 deferred — see STATE.md Deferred Items (real-iPhone smoke gate carries forward as v1.13 D-12 deferral, re-opened)
...
| DRAWER-05 | Phase 49 | Complete |
| DRAWER-06 | Phase 49 | Complete |
| DRAWER-07 | Phase 49 | Complete |
| DRAWER-08 | Phase 49 | Complete |
| DRAWER-09 | Phase 49 | Complete |
| DRAWER-10 | Phase 49 | Complete |
| DRAWER-11 | Phase 49 | Complete |
| DRAWER-12 | Phase 49 | Complete |
| DRAWER-TEST-01 | Phase 49 | Complete |
| DRAWER-TEST-02 | Phase 49 | Complete |
...
| REL-01 | Phase 51 | Complete (shipped under v1.16.x) |
| REL-02 | Phase 51 | Complete |
| REL-03 | Phase 51 | Complete (at milestone close) |
| REL-04 | Phase 51 | Partial (automated gates green; SMOKE-01..10 deferred to v1.16+) |
