...
**Plans**: 6 plans (3 shipped at Phase 4 closure 2026-04-26; 3 gap-closure plans shipped 2026-04-26 closing the F-03 hierarchy gap, the Oral hierarchy inversion gap, and the weight-in-Oral-mode safety-only UX gap surfaced in human UAT post-Phase-4-closure)
  ...
  - [x] 04-05-PLAN.md -- Wave 5 gap closure (D-12 Oral hierarchy promotion + D-13 Prescribing-Artifact-Leads Rule cross-mode codification: 3-token swap on `Estimated daily total (3 meals/day)` row at PertCalculator.svelte:213-227 to mirror Tube-Feed Wave 4 treatment; tracking-wide -> tracking-wider on eyebrow; text-base -> text-title + font-medium -> font-extrabold on numeral; PERT-route only; ~3 LOC; D-08 allowlist honored; Wave 4 Tube-Feed treatment byte-identical; includes `checkpoint:human-verify` gate enforcing D-13 audit-both-modes principle; closes UAT test #4) -- **COMPLETE 2026-04-26 (commits `f4565b4` Task 1 source edit + `618a66f` Task 4 SUMMARY)**: 3-token swap on Oral row at lines 213-227; PERT-route allowlist honored (only PertCalculator.svelte modified); all 9 plan-level gates green (svelte-check 0/0; vitest src/lib/pert/ 81/81; em + en-dash 0/0; tabular-numerals 9 unchanged; Wave 4 + Wave 2 Tube-Feed treatment preserved at `tracking-wider` = 2 + `font-extrabold` = 2; D-12 landed on the correct Oral row; function-binding bridges 3+; pert-config.json byte-identical; negative-space audit clean); cross-mode parity structurally complete (Oral and Tube-Feed prescribing-artifact rows share byte-identical eyebrow + numeral class strings); human-eye visual UAT verdict `approved` (verbatim, single-token) on light-desktop-oral + light-desktop-tube-feed in same UAT pass per D-13 audit-both-modes scope; 04-UAT.md test #4 gap row resolved by human verdict; PERT-DESIGN-02..05 stay Validated (gap row resolution flows through `/gsd-verify-work 4 --ws pert`); REQUIREMENTS.md NOT touched; user surfaced follow-up "weight isn't used in oral mode" UX concern -- xlsx + calc-layer audit confirms weight gates PERT-SAFE-01 max-lipase-cap safety advisory ONLY in Oral mode (not the dose); flagged as Wave 6 hand-off in 04-05-SUMMARY.md.
  - [x] 04-06-PLAN.md -- Wave 6 gap closure (D-14 weight-in-Oral safety-only helper text: mode-conditional `<p>` element added below the weight input in `PertInputs.svelte` SHARED card; gated `{#if pertState.current.mode === 'oral'}`; helper-text wording "Used for the 10,000 units/kg/day safety check, not the dose calculation." with ASCII-only chars; typography role `text-ui text-[var(--color-text-secondary)]` per Five-Roles-Only + Identity-Inside Rule; PERT-route only; ~5 LOC; D-08 allowlist honored; D-08b shared-component boundary held -- RangedNumericInput NOT touched; closes Wave 5 follow-up UX concern; resolves D-14) -- **COMPLETE 2026-04-26 (commits `b0fe1a0` Task 1 source edit + `a5ed7d7` Task 4 SUMMARY)**: +5 LOC sibling `<p>` element added between weight `RangedNumericInput` and SHARED card close at PertInputs.svelte lines 125-129; PERT-route allowlist honored (only PertInputs.svelte modified; RangedNumericInput shared component byte-identical); all 9 plan-level gates green (svelte-check 0/0; vitest src/lib/pert/ 81/81; em + en-dash 0/0; D-14 helper-text invariants landed at expected counts -- visible-text 1, mode-gate 2, class-string 1; Phase 3.1 D-01 function-binding bridges 3 + string-bridge proxies 0; Pitfall 4 selectors preserved -- `label="Weight"` + `sliderAriaLabel="Weight slider"` + `id="pert-weight"` byte-identical; PertCalculator.svelte byte-identical against HEAD -- Wave 5 D-12 + Wave 4 + Wave 2 frozen; pert-config.json byte-identical against HEAD -- Wave 1 commit `2dc7ae2` frozen; negative-space audit clean -- exactly 1 file in plan diff); cross-mode render confirmed (helper text visible in Oral mode; helper text hidden in Tube-Feed mode; orchestrator DOM-confirmed `helperPresent=false` in Tube-Feed); human-eye visual UAT verdict `approved` (verbatim, single-token) on light-desktop-oral + light-desktop-tube-feed in same UAT pass per the Wave-6 mode-conditional render check audit pattern (extending the Wave 5 D-13 audit-both-modes principle from typographic hierarchy assertions to mode-gated UI elements); Wave-5 weight-in-oral-mode UX hand-off gap row resolved by human verdict; PERT-DESIGN-02..05 stay Validated + PERT-SAFE-01 stays in current state (gap row resolution flows through `/gsd-verify-work 4 --ws pert`); REQUIREMENTS.md NOT touched; Identity-Inside Rule + Five-Roles-Only Rule + 11px Floor Rule preserved.
**UI hint**: yes
...
**Plans**: 1 plan (single autonomous plan per CONTEXT D-09; mechanical bookend phase)
  - [ ] 05-01-PLAN.md — Cleanup orphan UAT debug artifacts (D-08); 7-gate clinical gate (D-03 verification-only); bump package.json v1.13.0 -> v1.15.0 (D-01 + D-04 + D-04a); update workstream-local PROJECT.md + REQUIREMENTS.md + ROADMAP.md (D-05 + D-07); compose 05-01-SUMMARY.md with workstream-completion handoff (D-06 + D-09 Task 5)
...
---
*Last updated: 2026-04-26 -- Phase 5 plan 05-01 authored (single-plan release phase per CONTEXT D-09; cleanup + 7-gate clinical gate + version bump + workstream-local docs + SUMMARY); Phase 4 COMPLETE (Wave 3 clinical gate + score validation, plan 04-03; Wave 4 gap closure for F-03 Tube-Feed hierarchy issue, plan 04-04; Wave 5 gap closure for Oral hierarchy inversion + D-13 cross-mode parity codification, plan 04-05; Wave 6 gap closure for weight-in-Oral safety-only UX clarity, plan 04-06). All 6 Phase-4 plans shipped (`eec18c2..72be821` Wave 1; `29306e7`, `136b624` Wave 2 + out-of-band `2dc7ae2`, `5cd3386`; `95e31b0`, `3001c2a`, `33695e1`, `a6d9469` Wave 3; `d82aa30`, `78686f9` Wave 4; `f4565b4`, `618a66f` Wave 5; `b0fe1a0`, `a5ed7d7` Wave 6). Wave 3: aggregate 36.25/40; 8/8 at >= 35/40; AUDIT.sh exit 0; 18-gate verification all PASS; PERT-DESIGN-01..06 Active -> Validated. Wave 4: F-03 Approach C eyebrow `tracking-wide` -> `tracking-wider` shipped (combined with Wave 2 numeral `font-extrabold` = full COMBINED RECIPE); 9/9 plan-level gates green; human-eye visual UAT verdict `approved`; test #1 gap row resolved. Wave 5: D-12 Oral hierarchy promotion shipped via 3-token swap on `Estimated daily total (3 meals/day)` row at PertCalculator.svelte:213-227; D-13 Prescribing-Artifact-Leads Rule codified (workstream-local for v1.15; DESIGN.md update deferred to v1.16); cross-mode parity structurally complete; 9/9 plan-level gates green; human-eye visual UAT verdict `approved` on BOTH modes per D-13 audit-both-modes scope; test #4 gap row resolved. Wave 6: D-14 weight-in-Oral safety-only mode-conditional helper text shipped on PertInputs.svelte SHARED card (+5 LOC sibling `<p>` element gated `{#if pertState.current.mode === 'oral'}`; visible text `Used for the 10,000 units/kg/day safety check, not the dose calculation.`; class string `text-ui text-[var(--color-text-secondary)]`); Options 1 + 2 considered but rejected per 04-CONTEXT.md D-14 (Option 1: function-binding bridge regression risk; Option 2: clinical-safety regression -- making weight optional in Oral would silently disable PERT-SAFE-01); PERT-SAFE-01 referenced-not-modified (calc-layer + pert-config.json byte-identical; helper text is UX clarification of an existing safety check, NOT a calculation change); cross-mode render confirmed (helper visible in Oral; helper hidden in Tube-Feed -- orchestrator DOM-confirmed `helperPresent=false` in Tube-Feed); 9/9 plan-level gates green; human-eye visual UAT verdict `approved` on light-desktop-oral + light-desktop-tube-feed in same UAT pass per the Wave-6 mode-conditional render check audit pattern (extending the Wave 5 D-13 audit-both-modes principle to mode-gated UI elements); Wave-5 hand-off gap row resolved; PERT-DESIGN-02..05 stay Validated + PERT-SAFE-01 stays in current state (resolution flows through `/gsd-verify-work 4 --ws pert`). All gap-closure cycles for Phase 4 complete. Next phase: Phase 5 (Release).*
