...
- [x] **TEST-03**: New `webkit-iphone` Playwright project added to `playwright.config.ts` (using `devices['iPhone 14 Pro']` or equivalent) so e2e specs can execute under WebKit + iPhone viewport. Existing `chromium` project preserved unchanged. CI pipeline runs both projects.
