...
- [ ] **CORE-02**: User sees **Current GIR (mg/kg/min)** computed as `(Dex% × Initial rate ml/hr × 10) / (Weight × 60)` using exact constants (not spreadsheet's truncated `0.167`)
- [ ] **CORE-03**: User sees **Initial rate (ml/hr)** computed as `(Weight × ml/kg/day) / 24` as a secondary hero value
...
- [ ] **SAFE-05**: Input values are normalized on entry to tolerate EPIC paste: trim whitespace, convert locale comma to decimal point
...
- [ ] **ARCH-04**: New `src/lib/gir/` module contains `types.ts`, `gir-config.json`, `gir-config.ts` (typed wrapper), `calculations.ts` (pure functions), `state.svelte.ts` (rune singleton with dedicated `nicu_gir_state` sessionStorage key), `GirCalculator.svelte`, `GlucoseTitrationGrid.svelte`
...
- [ ] **TEST-01**: Spreadsheet-parity unit tests (`calculations.test.ts`) cover all 6 titration buckets × all formula columns (Current GIR, Target GIR, Target Fluids, Target rate, Δ rate) using frozen JSON fixtures extracted from the CALC tab (no live xlsx reads at test time); assertion tolerance ~1% epsilon to accommodate spreadsheet constant truncation
...
- [ ] **TEST-03**: Config shape tests (`gir-config.test.ts`) lock the JSON structure and range values
