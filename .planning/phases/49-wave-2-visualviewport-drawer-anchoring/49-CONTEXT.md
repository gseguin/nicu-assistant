...
- DRAWER-11: Existing `md:hidden` rule on the **`<InputsRecap>` trigger** (the only path that opens the drawer on mobile — `src/lib/shared/components/InputsRecap.svelte:162` declares `md:hidden`; the hidden trigger means `<dialog>.showModal()` never runs at md+ breakpoints) is preserved verbatim. The `<InputDrawer>` component itself has no `md:hidden` class — its mobile-only nature is structurally enforced by the trigger, not by a class on the dialog. iPad split-keyboard cases (PITFALLS.md P-06) are out of scope by architectural design.
...
- **D-13:** Edit `src/routes/+layout.svelte` lines 11-14 (the singleton imports) to add `import { vv } from '$lib/shared/visualViewport.svelte.js';`. Edit the `onMount` block at lines 52-71 to call `vv.init();` IMMEDIATELY AFTER `favorites.init();` on line 55 — i.e. as a new line 56. Note: `pwa.init()` is NOT called from this `onMount`; `pwa` is imported (line 14) but its update detection lives elsewhere (the `import('virtual:pwa-register')` block at lines 58-70 handles SW registration). The three existing `init()` calls in `onMount` are `theme.init()` (line 53), `disclaimer.init()` (line 54), `favorites.init()` (line 55) — Phase 49 adds a fourth (`vv.init()`) at line 56, mirroring the same pattern. Sequential ordering doesn't matter — singletons are independent — but matching the import order keeps the `onMount` block readable.
