...
- [ ] **Phase 47: Wave-0 — Test Scaffolding** — `window.visualViewport` polyfill in `src/test-setup.ts` (with self-test pattern mirroring lines 122–149); reusable `dispatchVisualViewportResize(height, offsetTop)` helper at `src/lib/test/visual-viewport-mock.ts`; new `webkit-iphone` Playwright project in `playwright.config.ts` (existing `chromium` project preserved). Closes blockers P-18 + P-19 before any feature code lands.
