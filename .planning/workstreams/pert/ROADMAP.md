...
**Plans**: 4 plans (3 shipped at Phase 4 closure 2026-04-26; 1 gap-closure plan authored 2026-04-26 to close the F-03 hierarchy gap surfaced in human UAT post-Phase-4-closure)
  ...
  - [ ] 04-04-PLAN.md -- Wave 4 gap closure (F-03 escalation to Approach C: eyebrow `tracking-wide` -> `tracking-wider` bump on Capsules per month row; combines with the Wave 2 numeral `font-extrabold` for a both-vectors typographic delta; PERT-route only; ~1 LOC; D-08 allowlist honored; F-03 numeral from commit 29306e7 byte-identical; includes a checkpoint:human-verify gate replacing the LLM-Design-Review fallback verdict from Phase 4 closure with human-eye review; escalation ladder (Approach D = numeral text-title -> text-[28px] / text-display; Approach E = visual isolation via background tint, top divider, or icon prefix) documented for the next planner pickup if Approach C is also insufficient by eye)
**UI hint**: yes
