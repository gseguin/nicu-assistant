...
**In progress:** v1.17 Remove PERT Calculator — Phases 52–53 complete (2026-05-23). **Phase 52** removed every byte of PERT source: `src/lib/pert/` (14 files) and `src/routes/pert/` deleted, `pertModule` dropped from the registry (5 calculators remain), `CalculatorId` union narrowed to 5 members, `pert` about-content key and `.identity-pert` CSS removed, both PERT e2e specs deleted, test suite repaired plus a T-21 regression test. **Phase 53** added the favorites safety net: SAFE-02 proves the existing `recover()` filter silently drops a stored `'pert'` favorite (order preserved) for v1.15+ upgraders, and SAFE-03 guards that first-run defaults stay `['feeds','formula','gir','morphine-wean']` with no `'pert'` — test-only, no source change (the filter already worked). Gates green: `pnpm build` OK, svelte-check 0/0, vitest 410/410, zero PERT references in `src/`+`e2e/`. Phase 52 verified 14/14; Phase 53 verified 4/5 automated (5th = browser-level visual check, auto-approved + carried as HUMAN-UAT per D-06). Next: Phase 54 (Documentation Cleanup + Release v1.17.0).
