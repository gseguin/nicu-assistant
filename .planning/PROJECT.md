...
**In progress:** v1.18 Persistence Seam — Phases 55–57 complete (2026-05-28..2026-05-29). **Phase 57** folded auto-persist-on-change behind `CalculatorStore`: the constructor now installs a single `$effect.root(() => $effect(() => { JSON.stringify(this.current); this.persist(); }))` (wrapped in `if (typeof localStorage !== 'undefined') {}` to skip SSR), inserted after `this.init()` so restore-then-install ordering is correct and the discarded cleanup callback reflects singleton-app-lifetime. Re-entry is impossible by construction (effect reads `this.current`, never `this.lastEdited.current`); the 60s `STAMP_DEBOUNCE_MS` is defense-in-depth. NINE duplicated `$effect(() => { JSON.stringify(...State.current); ...State.persist(); })` blocks were deleted — 5 from `*Inputs.svelte` (gir/morphine/fortification/feeds/uac-uvc) and 4 from `*Calculator.svelte` parents (gir/fortification/feeds/uac-uvc; morphine was already clean), the latter caught by the code review that flagged the plan's scope had missed the parent-file duplicates. Three new co-located tests pin the contract: (a) `.current` mutation triggers persist via `await tick()`, (b) class-only instance proves drawer-only-mount path (no component context required), (c) `vi.useFakeTimers()` deterministically crosses the 60s window asserting both halves of the debounce (within-window preserves stamp, beyond-window refreshes it). Gates: svelte-check 0/0 (4592 files), vitest 451/451 (+3), `pnpm build` OK. Verified 4/4; code review 0 critical (4 warnings — all addressed). **Phase 56** migrated the four shared global singletons onto the seam as thin behavior-preserving adapters: `theme.svelte.ts` (one `PersistentValue<string>` + `rawStringCodec` so the byte stays unquoted for the `app.html` FOUC read; init() keeps one allowed null-probe for the prefers-color-scheme fallback), `disclaimer.svelte.ts` (two `PersistentValue<string>` instances — v1 read-only to preserve the audit trail, v2 written on migration), `favorites.svelte.ts` (one instance with a custom codec that wraps `{v:1,ids}` plus the existing 6-step `recover()` passed as the seam's recover hook; `persist()` deleted; init() keeps one allowed null-probe for first-run write-back; `deserialize` throws loudly since recover owns the read path), and `lastEdited.svelte.ts` (per-instance `PersistentValue<number|null>` via a recover hook that guards `Number('')===0`→null; 60s debounce + stamp-before-write preserved in the class). After Phase 56 the only direct localStorage callers are the seam, `calculator-store.svelte.ts` (Phase 57), and the two intentional init() null-probes. The three regression suites (`favorites.test.ts`, `calculator-store.test.ts`, `DisclaimerBanner.test.ts`) stayed green with ZERO test edits. Gates: svelte-check 0/0 (4592 files), vitest 448/448 (+12), verified 9/9, code review 0 critical (4 warnings + 3 info; 5 addressed). Next: Phase 57 (Auto-Persist Behind CalculatorStore).

**Prior in milestone:** Phase 55 (2026-05-28) extracted the `PersistentValue<T>` seam: a new plain-`.ts` module (`src/lib/shared/persistent-value.ts`, zero imports, zero runes) exposing guarded `read`/`write`/`remove` — each behind a single `typeof localStorage === 'undefined'` guard + silent try/catch — with a per-instance codec defaulting to JSON plus a `rawStringCodec` (raw values stored without JSON quotes, preserving the `app.html` FOUC theme-read), and an optional `recover(raw: string | null) => T` migrate hook that owns the read path when present (expressive enough for both disclaimer v1→v2 and favorites 6-step recovery). Co-located tests (`persistent-value.test.ts`, 26 cases) are the single persistence test surface (SEAM-01..04): SSR guard ×3, write/remove silent-on-throw, parse-failure fallback, both representative recover fixtures, plus 4 edge-case guards added from code review (recover-throws, JSON literal "null", empty-string per codec). Behavior-preserving — no adapter migrated yet (theme/disclaimer/favorites/lastEdited migrate in Phase 56; `CalculatorStore` folds in Phase 57). Gates green: svelte-check 0/0 (4590 files), vitest 436/436 (+26), `pnpm build` OK. Verified 6/6; code review 0 critical. Next: Phase 56 (Migrate Shared Singletons).
...
---
*Last updated: 2026-05-29 — v1.18 Phase 57 complete: folded auto-persist behind `CalculatorStore` via a single `$effect.root()` in the constructor (SSR-guarded, discarded cleanup = app-lifetime singleton); deleted 9 duplicated `$effect(...persist...)` blocks (5 *Inputs.svelte + 4 *Calculator.svelte parents, the latter caught by code review). Three new tests pin the contract incl. a vi.useFakeTimers debounce test that crosses the 60s window. Re-entry impossible by construction. vitest 451/451, svelte-check 0/0, verified 4/4. v1.18 is now 3/4 phases shipped — next is Phase 58 (Release v1.18.0).*
